...
// Динамический импорт React Three Fiber компонентов
// Это предотвращает ошибки SSR
const Scene3D = dynamic(() => import("./scene-3d"), { ssr: false })

// Компонент чат-сообщения
function ChatMessage({ message, sender, visible }) {
  return (
    <motion.div
      ...
      className={`flex items-start gap-2 mb-2 ${sender === "bot" ? "justify-start" : "justify-end"}`}
    >
      {sender === "bot" && (
      ...
      <div
        className={`rounded-lg px-3 py-2 max-w-[80%] ${
          sender === "bot" ? "bg-gradient-to-r from-purple-600 to-cyan-600 text-white" : "bg-gray-800 text-white"
        }`}
      >
        {message}
      </div>
...
// Компонент голографического робота
function HolographicRobot({ isHovered }) {
  return (
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full">
        <Suspense fallback={<div className="w-full h-full flex items-center justify-center">Загрузка...</div>}>
          {/* Если Three.js доступен, используем его */}
          ...
          {/* Запасной вариант для мобильных устройств или если Three.js не загрузился */}
          <div
            className={`${typeof window !== "undefined" && window.innerWidth > 768 ? "md:hidden" : ""} w-full h-full`}
          >
            <HolographicRobot isHovered={isHovered} />
          </div>
        </Suspense>
      </div>
    </div>
